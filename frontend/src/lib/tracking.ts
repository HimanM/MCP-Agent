...
export function getTrackingSupportGuidance(tracking: TrackingSummary) {
...
export function getTrackingHeadline(tracking: TrackingSummary) {
  const details = describeTrackingStatus(tracking.status);
  if (tracking.estimated_delivery) {
    return {
      ...details,
      nextStep: `Expected around ${tracking.estimated_delivery}. ${details.nextStep}`,
    };
  }
  return details;
}
